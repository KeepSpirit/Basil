...
def MobileNetV2(input_shape, num_classes):
    ...
    output = tf.keras.layers.Reshape((num_classes,), name='output')(x)
    return tf.keras.models.Model(inputs, output)

    # inputs = tf.keras.layers.Input(shape=input_shape, name='input')
    # base = tf.keras.applications.mobilenet_v2.MobileNetV2(input_tensor=inputs, include_top=False, weights=None)
    # base.trainable = True
    # x = base.output
    # x = tf.keras.layers.GlobalAveragePooling2D()(x)
    # x = tf.keras.layers.Reshape((1, 1, 1280))(x)
    ...
    # x = tf.keras.layers.Conv2D(num_classes, (1, 1), padding='same')(x)
    # x = tf.keras.layers.Activation('softmax', name='final_activation')(x)
    # x = tf.keras.layers.Reshape((num_classes,), name='output')(x)
    # return tf.keras.Model(inputs=base.input, outputs=x)

    # input_s = tf.keras.layers.Input(input_shape)
    # base = tf.keras.applications.mobilenet_v2.MobileNetV2(input_tensor=input_s, classes=num_classes, include_top=False, weights=None, pooling='avg')
    # base.trainable = True
    # model = tf.keras.Sequential(base)
    # model.add(tf.keras.layers.Reshape((1, 1, 1280)))
    # model.add(tf.keras.layers.Dropout(0.3, name='Dropout'))
    # model.add(tf.keras.layers.Conv2D(num_classes, (1, 1), padding='same'))
    # model.add(tf.keras.layers.Activation('softmax', name='final_activation'))
    # model.add(tf.keras.layers.Reshape((num_classes,), name='output'))
    # return model
