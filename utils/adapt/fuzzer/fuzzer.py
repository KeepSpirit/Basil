...
class WhiteBoxFuzzer:
    ...
    def __init__(self, network, original_input, replacement_input, ground_truth, metric=None, strategy=None, k=10, delta=0.5, class_weight=0.5, neuron_weight=0.5, lr=0.1, trail=3, decode=None):
        '''Create a fuzzer.
        ...
        Args:
          ...
          original_input: An original input to test.
          replacement_input: A replacement input to test.
        ...
        self.original_input = np.array(original_input)
        self.replacement_input = np.array(replacement_input)
        ...
        self.original_input_label = None
        self.replacement_input_label = None
        self.orig_coverage = None
        self.covered = None
        self.coverage = None

    def start(self, hours=0, minutes=0, seconds=0, append='meta', verbose=0, original_pred_label=None):
        ...
        self.original_input_label = np.argmax(self.network.predict(np.array([self.original_input]))[1])
        internals, logits = self.network.predict(np.array([self.replacement_input]))
        orig_index = np.argmax(logits)
        orig_norm = np.linalg.norm(self.original_input)
        self.replacement_input_label = self.decode(np.array([logits]))
        self.covered = self.metric(internals=internals, logits=logits)
        self.orig_coverage = coverage(self.covered)

        # Initialize variables.
        self.archive = Archive(self.replacement_input, self.replacement_input_label, append=append)

        # Initialize the strategy.
        self.strategy = self.strategy.init(covered=self.covered, label=self.replacement_input_label)
        ...
        try:
            while True:
                # Create worklist.
                worklist = [tf.identity(np.array([self.replacement_input]))]
                ...
                while len(worklist) > 0:
                    ...
                    for _ in range(self.trail):
                        ...
                        distance = np.linalg.norm(new_input - self.original_input) / orig_norm
                        ...
                        if distance < self.delta:
                            if self.label != new_pred_label or new_pred_label != original_pred_label or new_pred_label != np.argmax(self.ground_truth):
        ...
        return np.array(ae_list), np.array(ae_target_list), self.replacement_input_label, np.array(ae_pred_label)
